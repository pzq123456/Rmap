...
# 加载充电桩位置数据和国家边界
charger_data <- read.csv("data/eu.csv")

country_boundary <- st_read("data/GeoJSON/eu.geojson")
...
density_matrix %>%
  ...
  add_shadow(ray_shade(density_matrix, sunaltitude = 30, sunangle = 100, zscale = 5), max_darken = 0.7) %>%
